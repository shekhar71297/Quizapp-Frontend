...
function BatchWiseStudent() {
  ...
  const addRegisterStaff = (event) => {
    ...
    if (state.errors.passwordError || state.errors.employeeIdError) {
      setState((prevState) => ({
        ...prevState,
        snackbarOpen: true,
        variant: 'filled',
  ...
  const { fname, lname, email, contact, password, password2, gender, dob, role, employeeId, isFieldsEnabled } = state;
  const isSubmitDisabled = !fname || !lname || !email || !contact || !password || !password2 || !gender || !role || !employeeId || !dob;
        ...
        <div>
          ...
          <form onSubmit={addRegisterStaff} action={<Link to="" />}>
            <TextField
              ...
              helperText={(state.errors.employeeIdError && validation.errorText('Please enter a valid student Email id')) || 'eg:john@example.com'}
